...
namespace AudioVisualizer
{
    public partial class ColorPicker : Form
    {
        ...
        private void huePanel_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                _selectedColor.H = Math.Clamp(e.Y / hueBounds.Height * 360, 0, 360);
                InvalidatePanels();
            }
        }
        ...
        private void satValPanel_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                _selectedColor.S = Math.Clamp(e.X / satValBounds.Width, 0, 1);
                _selectedColor.V = Math.Clamp(1 - e.Y / satValBounds.Height, 0, 1);
                InvalidatePanels();
            }
        }
